...
export function AppSidebar() {
  const { appVersion: themeAppVersion, setAppVersion: setThemeAppVersion } =
    useTheme();
  const { toggleSidebar, state: sidebarState } = useSidebar();

  const [appVersion, setAppVersion] = useState(
    projectConfig.defaultAppVersionId
  );
  ...
  const currentVersionDetails = projectConfig.availableAppVersions.find(
    (v) => v.id === appVersion
  );
  const currentVersionName =
    isClient && currentVersionDetails
      ? currentVersionDetails.name
      : projectConfig.availableAppVersions.find(
          (v) => v.id === projectConfig.defaultAppVersionId
        )?.name || "Loading...";
